...
namespace InfoPathXsnAnalyzer
{
    public class Program
    {
        public static void Main(string[] args)
        {
            if (args.Length == 0)
            {
                Console.WriteLine("Usage: InfoPathXsnAnalyzer.exe <path to .xsn file>");
                return;
            }

            string xsnFilePath = args[0];
            try
            {
                Console.WriteLine("Analyzing InfoPath form: " + xsnFilePath);
                Console.WriteLine("===========================================");

                EnhancedInfoPathParser parser = new EnhancedInfoPathParser();
                InfoPathFormDefinition formDef = parser.ParseXsnFile(xsnFilePath);

                // Display summary
                Console.WriteLine("\nForm Analysis Summary:");
                Console.WriteLine($"- Total Views: {formDef.Views.Count}");
                Console.WriteLine($"- Total Controls: {formDef.Metadata.TotalControls}");
                Console.WriteLine($"- Total Sections: {formDef.Metadata.TotalSections}");
                Console.WriteLine($"- Dynamic Sections: {formDef.Metadata.DynamicSectionCount}");
                Console.WriteLine($"- Repeating Sections: {formDef.Metadata.RepeatingSectionCount}");
                Console.WriteLine($"- Unique Data Columns: {formDef.Data.Count}");

                if (formDef.Metadata.ConditionalFields.Any())
                {
                    Console.WriteLine($"- Conditional Fields: {string.Join(", ", formDef.Metadata.ConditionalFields)}");
                }

                // Show final JSON
                string json = JsonConvert.SerializeObject(formDef, Formatting.Indented);
                Console.WriteLine("\nFinal JSON with enhanced structure:");
